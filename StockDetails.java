...
public class StockDetails //to find total volume of each set
{
  ...
  public static class TokenizerMapper extends Mapper<LongWritable, Text, Text, LongWritable>
  {
	  ...
	  public void map(LongWritable key, Text value, Context context)
	  {
		  try{
			  ...
			  context.write(new Text(str[5]), new LongWritable(vol));
		  }
		  catch(Exception e)
		  {
			  e.printStackTrace();
  ...
  public static void main(String[] args) throws Exception {
    ...
    Job job = Job.getInstance(conf, "StockDetails");
    job.setJarByClass(StockDetails.class);
